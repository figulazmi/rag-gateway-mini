...
| Chunk count | `knowledge_v2_keyfacts` 539 post-cleanup corpus; legacy `knowledge_v2` 441 | Live Qdrant verification on 2026-05-06 after artifact cleanup; keyfacts still contains all legacy points and retained keyfacts-only records |
...
> **Status update (2026-05-06):** T1-A/B/C and T2-B miss cleanup remain complete. A final 47-query P2.6 evidence pass kept semantic judge off and confirmed `knowledge_v2_keyfacts` still leads legacy on Hit@1, MRR, NDCG@5, and p95 latency while gateway smoke and negative confidence gating passed. VM B1 production targets `knowledge_v2_keyfacts`, and the verified post-cleanup corpus state is 539 keyfacts points versus 441 legacy points.
