...
| **Hallucination Rate** | Claims with zero chunk grounding | ≤ 5% | 71.4% on 7/7 grounded end-to-end smoke with no skipped cases (2026-05-07) | `[ ] NOT MET` |
...
**Hallucination grounded smoke (2026-05-07):** `scripts/eval-retrieval-quality.py` now supports `--embed-timeout` and `--generate-timeout` for slow VM B1 Ollama runs, caps end-to-end generation to 128 tokens, records skipped generation cases in JSON, and uses a grounded prompt that requires an exact command, endpoint, identifier, or code fragment from retrieved context. Added `expected_snippet` coverage for liveness, rollback-safe keyfacts, and full ingest default fixtures. Baseline coverage report `.claude/reports/quality-hallucination-coverage-2026-05-07.json` had 7 eligible cases, 4 completed, 3 skipped, and 4/4 hallucinated (`100.0%`). Grounded report `.claude/reports/quality-hallucination-grounded-2026-05-07.json` completed all 7 eligible cases with no skips and reduced hallucination to 5/7 (`71.4%`). Hybrid retrieval stayed strong (`Hit@1=1.0000`, `MRR=1.0000`) while generation remains the active Quality gap.
