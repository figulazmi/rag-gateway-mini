...
| 5 | P3.2 | Expand eval set and implementation correctness test | Includes retrieval eval expansion and end-to-end strategy; excludes future semantic judge replacement | `[x] DONE (2026-05-10)` | Added `scripts/eval-fixtures/homelab-expanded.json`; 28 -> 26 fixture entries after cleanup. 9 fixtures carry `expected_ids`; 2 negative fixtures marked `negative: true`. Smoke design spec at `docs/superpowers/specs/2026-05-10-p32-smoke-design.md`. Captured and pushed 4 implementation-spec chunks to knowledge_v2_keyfacts (2 for /rag/search contract, 2 for /rag/answer contract; total corpus 597 points). Automated harness at `scripts/smoke-p32-implementation-correctness.py` merged from feat/p32-smoke-harness. /rag/search smoke passed: gateway returned `status=found` with top score 0.833, build exited 0 with 0 warnings, API smoke HTTP 200. Report at `.claude/reports/p32-smoke-2026-05-10-retry.json`. /rag/answer smoke initially failed (HTTP 500 Ollama timeout); fixed by adding configurable `GenerationTimeoutSeconds` to RagGatewayOptions (default 180s, VM B1 override 600s) and wiring it into Program.cs OllamaGenerationClient HttpClient timeout. Retry passed: HTTP 200 found. Report at `.claude/reports/p32-smoke-2026-05-10-rag-answer-retry.json`. | None |
